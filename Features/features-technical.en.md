...
- Cloud and Twilio WhatsApp look up the exact phone first, then Brazilian ninth-digit candidates only for legacy eight-digit subscribers starting with 6–9. Lookup is account-scoped and does not change the stored phone. Names containing an equivalent phone can be enriched from the profile. Cloud calls use the same builder; historical source_id variants and Evolution/WAHA/Uazapi rules are preserved.
...
- Cloud registration recovery: `POST /api/v1/accounts/:account_id/whatsapp/phone_registrations/request_code` requests SMS/VOICE; `/verify_code` validates the `code` field; `POST /phone_registrations` sends only `pin` to `/register`. Inbox update permission is required and Meta errors are preserved. Received codes are never stored as PINs. Without a stored PIN, setup configures webhooks and leaves registration awaiting user action. PINs are saved after successful registration; `133005` directs PIN recovery in Meta and attempt limits are never treated as success. Coexistence skips this registration. Explicit connection refresh reads the actual `status` without reusing cache, and business enrichment queries `waba_id` with a legacy-ID fallback. References: [registration](https://developers.facebook.com/documentation/business-messaging/whatsapp/business-phone-numbers/registration/), [verification and registration](https://developers.facebook.com/documentation/business-messaging/whatsapp/solution-providers/registering-phone-numbers/), and [Meta errors](https://developers.facebook.com/documentation/business-messaging/whatsapp/support/error-codes/). If registration is accepted but webhook setup fails, the response is `422 webhook_setup_failed`, the accepted PIN is retained, and `webhook_pending` remains; a subsequent attempt with a connected number repairs only the webhook.
...
- On Cloud, `whatsapp_embedded_signup_inbox_creation` gates only new embedded-signup inbox creation. Administrators can reconfigure or reauthorize existing embedded-signup WhatsApp Cloud inboxes without a feature flag; `whatsapp_reconfigure` remains deprecated to preserve stored flag positions.
...
- WAHA group-message edits reuse the group-member mention composer. Stored mention Markdown, or edited `@phone` text resolved from loaded group members, restores the member name in the editor; the timeline uses the same resolution for display. A valid selected member is sent through the native WAHA edit request as normalized text plus `mentions` JIDs. Direct chats, unsupported providers, and malformed mention markup are rejected.
- Incoming group edits from Evolution, WAHA, and Uazapi preserve an existing bold `number - name` participant header beneath the localized edit marker; repeated edits and provider-prefixed bodies remain deduplicated. Incoming text edits from WhatsApp Cloud and Notificame update the stored message by its provider ID, preserve its original content, and use the same localized marker without creating a duplicate.
...
- Facebook, Instagram, and TikTok channels persist `provider_name` on creation or reauthorization and expose it in the inbox payload; reauthorization preserves a custom inbox name. The `social_channels:backfill_provider_names` task starts in dry-run mode and supports resumable provider-scoped batches for existing channels.
...
- WaVoIP with per-inbox session persistence and role-aware credential reuse. During initialization, it requests the inbox ICE configuration and supplies the STUN/TURN servers from `Mega::Calls::IceConfig` to the WebRTC client when the account has `premium_call_connectivity`; without that configuration, it retains the provider's default behavior.

### 3.2 Conversation core

- `ReplyBox` registers shortcuts through `useKeyboardEvents`, whose `AbortController` removes the listener on unmount, including components with multiple Vue roots. Send validation, closing mentions with Escape, and the editor reference used to restore focus after resizing are preserved; `FullEditor` no longer includes the obsolete keyboard mixin.
...
- Selected conversations can be marked read or unread through one synchronous account-scoped batch endpoint. It filters display IDs with `Conversations::PermissionFilterService`, preserves per-user notification state, returns the affected read-state rows, and invalidates sidebar unread counts once.
...
- Cloud billing records a scheduled Stripe subscription cancellation and exposes its date to Billing. Cancellations end at the current billing-period boundary, including Stripe flexible billing payloads that use `cancel_at`; an account deletion queues the same cancellation safely for retry.
...
- Conversation assignment: the `captain_integration` account feature exposes only the Captain configured for every requested inbox to the typed assignment selector. Assigning Captain makes the conversation pending; clearing Captain or replacing it with a human opens it. Accounts without the feature cannot select or assign Captain, while the legacy `include_agent_bots` request remains limited to Agent Bots.
...
- Intercom and Freshdesk imports are administrator-only and feature-gated by `data_import`; source credentials and durable mappings are stored per account.
...
- Selector counts skip offer-value calculations. Full totals project only the required offers and currency; search/filter cache keys reuse preloaded conversations, and reports preload funnels and omit unused columns. Calendar reminders filter candidates by their configured lead time before loading and locking events, preserving the final locked check and cron frequency.
...
- `KanbanItems::ChecklistService` normalizes checklist keys when scheduling alerts. After saving a duplication, it schedules only copied entries with their new IDs and the destination card ID; merging lists does not reschedule existing entries. Missing dates or assignees do not schedule alerts, and past deadlines enqueue for immediate execution.
...
- The JSON `appContext` event preserves `conversation`, `contact`, and `currentAgent`, adding `customAttributes` from `attributes/getAttributes` and `theme` (`light`/`dark`) from the `body` class. Definition changes (including store mutations), theme changes, and tab reopening resend context to visible loaded frames. `chatwoot-dashboard-app:fetch-info` identifies the requesting iframe by `event.source`; unmounting removes the listener and theme observer.
...
- 2FA/MFA, SAML/SSO, custom roles, and audit logs. SAML rejects invitations and authentication for users associated with more than one account, and bulk provider changes only affect users exclusive to the account. Message deletion locks the row, snapshots its original content and conversation, inbox, and sender context, then writes its Enterprise audit entry in the same transaction as the soft deletion; repeated deletes do not create duplicates. Audit-log list responses omit Message payloads and the retained content.

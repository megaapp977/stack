...
Las grabaciones de Twilio Voice permanecen adjuntas a `Call`; una configuración explícita de cuenta habilita su transcripción idempotente, transmisión del mensaje e indexación en búsqueda.
...
- Métricas opcionales de colas Sidekiq hacia CloudWatch: `ENABLE_SIDEKIQ_CLOUDWATCH` inicia el reporte solo en el proceso servidor de Sidekiq. Usa el rol de instancia por defecto (o credenciales dedicadas `SIDEKIQ_CLOUDWATCH_AWS_*`) y publica QueueLatency, QueueSize, EnqueuedJobs y Utilization en un intervalo positivo validado.
...
- Los identificadores públicos de los inboxes se resuelven en frontend por tipo de canal y se muestran junto al nombre en la barra lateral, la lista y la cabecera de configuración. Los IDs de página de Facebook y de perfil de X son valores opacos de enrutamiento, por lo que se excluyen de la visualización aunque permanecen en el payload del inbox para operaciones del proveedor. La búsqueda combina coincidencias de nombre/tipo con los identificadores normalizados visibles, sin duplicar resultados y preservando la dirección de texto mixta con `bdi`.
...
- Mega Hub para Meta: modo opcional por Super Admin para conectar WhatsApp, Messenger e Instagram usando apps compartidas del Hub; el bloque de credenciales del Hub se configura en Super Admin → Mega Hub, y los inboxes creados siguen enviando por los servicios nativos y reciben eventos reenviados por webhook. La salud de WhatsApp valida el forward del Hub en lugar de comparar el callback de Meta hacia el Hub con el callback directo de MEGA, y solo informa un fallo de configuración cuando confirma que el forward es inválido. La pantalla final trata Mega Hub como registro integrado y no muestra instrucciones manuales de URL del webhook ni token de verificación.
...
- Salud de la cuenta consulta el perfil comercial de WhatsApp bajo demanda para bandejas Cloud API y lo devuelve sin persistirlo. Un fallo en esa consulta se registra y no oculta los datos principales del número ni del negocio.
...
- La configuración manual guiada de WhatsApp crea una nueva bandeja Cloud desde un ID de WABA, ID de número de teléfono y token permanente validados. Guarda el WABA bajo ambas claves de compatibilidad, configura explícitamente el callback a nivel de teléfono, expone comprobaciones de callback/suscripción y evita el callback genérico posterior a la creación.
...
- El registro integrado acepta finalizaciones de coexistencia con WhatsApp Business App sin `phone_number_id`; reenvía `is_coexistence` e `is_business_app_onboarding` para que el servicio de autorización configure el canal y los webhooks de ese flujo.
- WhatsApp Evolution, WAHA y Uazapi: proveedores alternativos con soporte multimedia y grupos.
- Las ediciones entrantes de grupos desde Evolution, WAHA y Uazapi conservan un encabezado existente en negrita `número - nombre` bajo el marcador localizado de edición; las ediciones repetidas y los cuerpos ya prefijados por el proveedor no duplican encabezados.
...
- Proxy de sesión WAHA: la configuración se aplica mediante `config.proxy` al actualizar la sesión, con `server`, usuario y contraseña opcionales; puede eliminarse con `proxy: null`.
- Proxy de Uazapi: la configuración de la bandeja admite un proxy interno regional con ciudad brasileña validada desde el catálogo, un proxy personalizado sin fallback o la ausencia de proxy; cambiar o quitar el proxy aplica el modo correspondiente del proveedor y quitarlo borra la preferencia regional guardada.
...
- Descarga diferida de multimedia de grupos WAHA (lazy media): una bandeja puede activar `lazy_media_download` para persistir solo la multimedia entrante de grupos como adjunto pendiente (solo identificadores del proveedor en `meta`); un agente la descarga bajo demanda mediante el endpoint de conversación `download_attachment`, que reutiliza el flujo de dos pasos metadata-then-binary y adjunta el archivo de forma idempotente. Las conversaciones individuales conservan la descarga inmediata.
...
- La autorización de TikTok se aplica mediante la funcionalidad de cuenta `channel_tiktok`; las cuentas alojadas sin acceso ven el acceso para solicitar soporte y el onboarding excluye TikTok de las sugerencias detectadas.
...
- El endpoint de mensajes del widget devuelve `403 Forbidden` para envíos de texto y adjuntos a una conversación resuelta cuando `inboxes.allow_messages_after_resolved` es falso. Esta protección del servidor también cubre reintentos de mensajes fallidos y mantiene la conversación resuelta sin cambios.
- Estados: open, pending, resolved, snoozed.
- Los timestamps relativos del dashboard usan `exactTimestamp` para un tooltip diferido con la fecha y hora completas; `TimeAgo` muestra creación y última actividad en líneas separadas.
...
- Ejecución de macros y atributos requeridos: el editor de respuesta ofrece un selector con `#`, mientras la barra de comandos muestra **Ejecutar una macro** solo en rutas de conversación activas con la función de macros habilitada; ambos usan el mismo orden persistido de la barra lateral. Las tres superficies comparten ejecución, detectan `resolve_conversation` y `change_status` a resuelta, solicitan y persisten valores faltantes antes de ejecutar, y permiten continuar sin resolver al cerrar el diálogo. El overlay de `Macros::ExecutionService` también bloquea la resolución directa cuando faltan valores de definiciones requeridas vigentes; una casilla `false` cuenta como completa y los valores nulos como faltantes.
...
- La interfaz heredada de historial y la ruta `/notifications` se eliminan, mientras `inbox_view` permanece deshabilitado. Las notificaciones persistidas siguen alimentando popups realtime, contadores del sidebar y reconciliación tras reconectar. Calendario usa su payload Push especializado; `kanban_note_agent_assigned`, `kanban_stage_automation`, `kanban_checklist_due_date` y `account_task_due` usan un payload operativo separado con URLs directas y sin entrega por email.
...
- El selector de respuestas predefinidas se teletransporta y ancla al cursor, carga la lista de la cuenta mediante la caché IndexedDB endurecida, filtra atajos/contenido localmente, reconcilia invalidaciones en tiempo real, previsualiza contenido compatible con el canal y conserva placeholders Liquid sin resolver para que los evalúe el backend.
...
- Las reglas diferidas persisten una ejecución pendiente reclamada por regla, conversación y episodio de estado. El editor conserva las condiciones estructurales de la espera y admite condiciones adicionales unidas con `AND`; un worker programado vuelve a validar la feature flag, la regla y todas las condiciones antes de ejecutarla. Los cambios de estado y las respuestas invalidan el episodio correspondiente.
...
- Assistants: configuracion por inbox con instrucciones y contexto. `config.auto_resolve_mode` se persiste por asistente como `evaluated`, `legacy` o `disabled`; los asistentes existentes que no lo tienen usan el modo de cuenta como compatibilidad. Con Captain V2, `config.auto_resolve_after` acepta de 5 a 1.440 minutos y se normaliza en incrementos de cinco, mientras `send_inactivity_resolution_message` controla el mensaje público. Un árbol validado y opcional de condiciones de audiencia y `response_window` (`always`, `business_hours` u `outside_business_hours`) deciden la elegibilidad al crear o reabrir conversaciones; las que ya están pendientes continúan sin interrupción. La API fusiona de forma segura las actualizaciones de configuración y serializa los valores efectivos.
- La reconciliación de facturación cloud habilita `captain_integration_v2` para todo plan que no sea el predeterminado y lo deshabilita para el plan predeterminado configurado. No depende de un atributo de despliegue por cuenta; el runtime de Captain V1 y la bandera de funcionalidad siguen disponibles.
...
- Resumen del asistente: los endpoints Enterprise heredados de estadisticas, drilldown y resumen cacheado se mantienen basados en `Captain::AssistantStatsBuilder`, `Captain::AssistantStatsWindow`, `Captain::AssistantDrilldownBuilder` y `Captain::OverviewSummaryService`. La bandera local `captain_overview_v2` selecciona una interfaz rediseñada y opcional que consume los builders dedicados de resultados, flujo y tendencia de resolución; la tendencia incluye tasas alineadas del periodo actual y anterior, mientras el CSAT exclusivo humano no disponible permanece en nil en vez de mostrar una puntuación falsa. `overview_summary` genera en el servidor hasta tres insights localizados y redondeados, omite informes sin actividad y comparte en Redis una caché exitosa de una hora por cuenta, versión del asistente, rango, zona horaria e idioma. El tiempo ahorrado estimado se deriva de las respuestas públicas del asistente usando una suposición fija de 2 minutos de esfuerzo de agente por respuesta.
- El routing de modelos Captain separa las funciones de cliente e internas en Super Admin. El evaluador interno `conversation_completion` acepta modelos OpenAI compatibles; su ruta resuelve la sobrescritura de cuenta, luego el modelo de instalación autohospedada y finalmente el valor predeterminado de YAML. Las preferencias de Captain para clientes continúan exponiendo solo funciones de cliente.
- Editor de respuesta de Copilot: mide las sugerencias renderizadas después de actualizar el DOM y crece temporalmente para mostrarlas, con un máximo de 350 px. El redimensionamiento manual conserva prioridad y se restaura al cerrar la sugerencia; los estados de carga y sugerencia hacen cross-fade en su lugar.
- Sugerencias de respuesta de Copilot: una solicitud explícita valida que el agente acceda a la conversación y el último mensaje público sea entrante. Un job dedicado usa el prompt, FAQs, citas y únicamente custom tools GET del asistente, sin escenarios ni handoff; vuelve a validar acceso y actualidad antes de persistir, descarta borradores obsoletos y consume crédito sólo al guardar una respuesta válida.
...
- Captain Documents: carga, indexacion y auto-sincronizacion por plan con jitter, cola purgable, limites configurables por cuenta y globales, y una vista de detalles con contenido rastreado, metadatos de origen, recuento de preguntas frecuentes generadas y uso en conversaciones exclusivo para administradores.
- Analítica de uso de conocimiento: `Captain::ConversationUsageBuilder` calcula conversaciones activas distintas desde sesiones del asistente con respuesta entregada (`credits_consumed > 0`) usando índices GIN sobre `document_ids` y `used_faq_ids`; pagina los drilldowns de 25 registros, excluye handoffs y conversaciones eliminadas, permite ordenar documentos por uso y expone el uso de FAQ creadas por usuarios solo a administradores.
- Captain Scenarios: reglas de activación y prioridad; las API de administración devuelven registros habilitados y deshabilitados, mientras que solo los habilitados se ejecutan. La API conserva `tools` enviados explícitamente, normaliza IDs escalares o metadata `{ id }`, acepta referencias guardadas a herramientas personalizadas deshabilitadas y mantiene referencias `tool://` cuando se omite el campo. Account MCP publica schemas específicos para crear y actualizar escenarios.
- Captain Custom Tools: integraciones HTTP con GET, POST, PUT, PATCH y DELETE; admiten fragmentos JSON Schema para parámetros complejos y Account MCP publica contratos directos para listar, crear, consultar, actualizar, eliminar y probar tools personalizadas. La respuesta de detalle informa cuántos escenarios habilitados usan una herramienta, para que el dashboard advierta antes de desactivarla; las herramientas deshabilitadas se excluyen de la ejecución.
...
- Auto-resolve mode: evaluated, legacy o disabled por asistente. El modo evaluado envía al evaluador el estado de la conversación y el contenido etiquetado de los mensajes no privados; las rutas de resolución y handoff bloquean y recargan la conversación antes de transicionar, crean mensajes en esa transacción y emiten eventos solo después de tener éxito. Las transferencias y seguimientos pendientes se mantienen abiertas.
...
- La entrada del sidebar de Empresas y su autorizacion CRUD estan disponibles para administradores, agentes estandar y roles personalizados con el permiso `companies_manage`.
...
- La exportacion CSV de contactos usa `CSVSafe` para prefijar valores que comienzan con caracteres de formula y evitar su ejecucion en hojas de calculo.
...
- Editor con menu slash, tablas nativas, cargas de imágenes en segundo plano (progreso, cancelación y reintento) y cargas MP4 o enlaces de video compatibles; el medio mantiene su posición durante la carga y las dimensiones del video persisten en el editor y el portal público.
...
- `ListTab` consume `GET /kanban_items` en páginas de 30, reemplaza el estado en la primera página y agrega ítems únicos en las siguientes; al cambiar de funnel reinicia tarjetas y metadatos antes de pedir la nueva página inicial.
- `GET /kanban_items/report_export` usa el scope de políticas Kanban y los filtros del reporte sin paginación. Emite CSV UTF-8 con BOM y filas CRLF mediante Ruby CSV, encabezados/prioridades localizados, nombres configurados de etapa y moneda explícita del ítem antes de la moneda de la cuenta.
...
- En instalaciones alojadas, el acceso mediante tokens de API y la entrega saliente de webhooks requieren la funcionalidad de cuenta `api_and_webhooks`; las instalaciones autogestionadas permanecen habilitadas por defecto.
...
- Slack admite `two_way` (predeterminado) y `alert`; este último mantiene la sincronización hacia Slack pero descarta las respuestas entrantes del hilo para que no lleguen al cliente.
...
- Los assets de aplicación PWA y splash se generan dinámicamente desde `LOGO_THUMBNAIL`, rasterizando SVG mediante librsvg/libvips y con fallback a `NOTIFICATION_ICON`; las alertas Push del sistema y badges usan `NOTIFICATION_ICON` (por defecto `/favicon-badge-16x16.png`) con el fallback recíproco, pero el badge monocromático no se anuncia como icono instalable. El splash HTML se retira dos frames después del primer render de Vue, con transición de 180 ms y límite de seguridad de 4 s. El fondo es configurable y la caché se invalida por asset, color y timestamp del blob; el favicon conserva la imagen de aplicación, cambia a la de notificaciones con la pestaña oculta o sin foco y se restaura al regresar.
- La confiabilidad PWA excluye la navegación autenticada de Cache Storage, limita los assets Vite con fingerprint, hace que Rails sirva `/sw.js` con cabeceras de revalidación obligatoria, reconcilia el estado realtime solo después de confirmar `RoomChannel` y mantiene el ciclo de opt-out/logout/cambio de clave VAPID por usuario y dispositivo; la activación acepta claves VAPID Base64 o binarias inicializadas por Rails y diferencia navegador incompatible, permiso bloqueado, configuración ausente, sesión vencida y fallo de registro. Los proxies inversos y CDN deben preservar las cabeceras del worker.
...
- 2FA/MFA, SAML/SSO, roles personalizados y logs de auditoria. SAML rechaza invitaciones y autenticación de usuarios asociados a más de una cuenta, y los cambios masivos de proveedor solo afectan usuarios exclusivos de la cuenta. El informe de evidencia de eliminaciones de Super Admin consulta únicamente filas retenidas de `audits`: la destrucción de Inbox usa su asociación con Account, mientras que las de Conversation y Contact usan la captura de `account_id` en `audited_changes`; nunca une registros vivos eliminados ni infiere eliminaciones de Message.
- El endpoint Enterprise de logs de auditoría acepta los filtros opcionales `q`, `types[]`, `since`, `until` y `sort`; las marcas de tiempo se limitan a épocas Unix seguras para la base y el índice de asociación de cuenta más fecha soporta el ordenamiento filtrado. Los filtros del dashboard viven en la URL y descartan respuestas obsoletas.
...
- `POST /super_admin/users/:id/resend_confirmation` solo está disponible para Super Admin; para usuarios sin confirmar vuelve a enviar las instrucciones estándar de Devise y para usuarios confirmados no encola correo. La vista de detalle oculta la acción cuando el usuario ya está confirmado.
